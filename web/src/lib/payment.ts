...
export async function createPayment(req: PaymentRequest): Promise<PaymentResponse> {
    ...
    try {
        ...
        const params = new URLSearchParams({
            ...
            merchant_ok_url: process.env.PAYTR_OK_URL || `${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/payments/success`,
            merchant_fail_url: process.env.PAYTR_FAIL_URL || `${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/payments/fail`,
            timeout_limit: '30',
            currency: 'TL',
